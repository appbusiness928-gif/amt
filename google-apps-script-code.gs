 ...
 * เชื่อมต่อกับ Sheet ID: 1H4eo1newu0vTP0ura2xyQ-xBixDVId-yLY8G3dNId2c
 * 
 * ลิงก์สเปรดชีต: https://docs.google.com/spreadsheets/d/1H4eo1newu0vTP0ura2xyQ-xBixDVId-yLY8G3dNId2c/edit?usp=sharing
 * 
 * วิธีการติดตั้ง:
 * 1. เปิด Google Sheets (ID: 1H4eo1newu0vTP0ura2xyQ-xBixDVId-yLY8G3dNId2c)
...
const SPREADSHEET_ID = "1H4eo1newu0vTP0ura2xyQ-xBixDVId-yLY8G3dNId2c";

// รายชื่อชีตและหัวข้อหลัก (มีครบถ้วนสมบูรณ์ตาม Typescript interface)
const SHEETS_CONFIG = {
  "Users": ["id", "photoUrl", "title", "firstNameTh", "lastNameTh", "role", "signature", "email", "phone", "status", "createdAt", "batch", "password", "languagePreference"],
  ...
  "RoomUsageRecords": ["id", "date", "room", "requesterName", "report", "maintenanceOfficerStatus", "remarks", "requesterSignature"],
  "Equipment": ["no", "toolName", "partNumber", "serialNumber", "code", "qty", "location", "status", "remark", "calibrationDate"],
  "BorrowRecords": ["id", "equipmentCode", "toolName", "borrowerId", "borrowerName", "borrowerRole", "qty", "borrowDate", "status", "purpose", "returnDate", "borrowSignature", "toolLocation", "returnSignature", "checkSignature", "checkerName"],
  ...
  try {
    ...
    if (payload.action === "syncData" && payload.data) {
      ...
      if (payload.htmlContent) {
        ...
        const htmlOutput = HtmlService.createHtmlOutput(styledHtml);
        fileBlob = htmlOutput.getAs("application/pdf");
...
/**
 * ฟังก์ชันสำหรับทดสอบการทำงานภายใน Apps Script IDE
 * สามารถกดเลือกชื่อฟังก์ชัน "runTestSetup" แล้วกดปุ่ม "Run" (เรียกใช้) ด้านบน
 * เพื่อทดสอบการเชื่อมต่อกับสเปรดชีต และสร้างคอลัมน์ตารางทั้งหมดเริ่มต้นโดยอัตโนมัติ!
 */
function runTestSetup() {
  Logger.log("====================================================");
  Logger.log("เริ่มต้นทดสอบสคริปต์ระบบ AMT CONNECT...");
  Logger.log("====================================================");
  try {
    ensureSheetsExist();
    Logger.log("✅ สร้าง/ตรวจสอบตารางชีตทั้งหมดใน Google Sheets เรียบร้อยแล้ว!");
    
    // ทดสอบดึงข้อมูล
    const data = fetchAllSheetsData();
    Logger.log("✅ ตรวจสอบการดึงข้อมูลสำเร็จ! พบรายการข้อมูลดังนี้:");
    for (let key in data) {
      Logger.log("   - ชีต " + key + " มีข้อมูล: " + data[key].length + " แถว");
    }
    Logger.log("====================================================");
    Logger.log("🎉 การทดสอบเสร็จสิ้นอย่างสมบูรณ์แบบ! สคริปต์ของคุณพร้อมใช้งาน 100%");
    Logger.log("====================================================");
  } catch (err) {
    Logger.log("❌ เกิดข้อผิดพลาดในขั้นตอนทดสอบ: " + err.toString());
    Logger.log("คำแนะนำ: กรุณาตรวจสอบว่า SPREADSHEET_ID หรือสิทธิ์การเข้าถึงถูกต้อง");
  }
}
