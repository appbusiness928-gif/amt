...
export const DEFAULT_GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxQec6a3Nb-QeJnueUwgZ1XC3f1GUe6xYSRr5JlU9qFXFUg8NF3clknj-WObhUouPnBxA/exec';
...
export async function pullFromGoogleSheets(): Promise<any | null> {
  if (isPulling) {
    console.log('Already pulling from Google Sheets, skipping concurrent call.');
    return null;
  }
  isPulling = true;
  try {
    ...
    if (err.name === 'AbortError') {
      console.warn('Google Sheets pull request timed out and was aborted.');
    } else {
...
export function clearLocalStorageData(): void {
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('amt_conn_')) {
